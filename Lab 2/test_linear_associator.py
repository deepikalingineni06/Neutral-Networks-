...
def test_weights():
    ...
    assert np.array_equal(model.get_weights(), np.ones(
        (number_of_nodes, input_dimensions)))
    ...
    np.testing.assert_array_almost_equal(
        model.get_weights(), weights, decimal=3)
...
def test_train_linear_delta():
    ...
    for i in range(10):
        ...
        model.train(X_train, out, batch_size=10, num_epochs=50,
                    alpha=0.1, gamma=0.1, learning="Delta")
        new_out = model.predict(X_train)
...
def test_train_hardlim_delta():
    ...
    for i in range(10):
        ...
        out = model.predict(X_train)
        model.set_weights(np.random.randn(*model.get_weights().shape))
        model.train(X_train, out, batch_size=10, num_epochs=100,
                    alpha=0.1, gamma=0.1, learning="Delta")
        new_out = model.predict(X_train)
        np.testing.assert_array_almost_equal(out, new_out, decimal=4)
...
def test_calculate_mean_squared_error():
    ...
    assert model.calculate_mean_squared_error(
        X_train, model.predict(X_train)) == 0
