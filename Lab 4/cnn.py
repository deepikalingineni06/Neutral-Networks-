...
class CNN(object):
    ...
    def append_dense_layer(self, num_nodes, activation="relu", name="layer1", trainable=True):
        """
        ...
        for n in range(0, N):
            names = list(layer.name for layer in self.model.layers)
            if not name in names:
                break
            name = name[-1] + str(n + 1)

        self.model.add(keras.layers.Dense(units=num_nodes,
    ...
    def append_conv2d_layer(self, num_of_filters, kernel_size=3, padding="same", strides=1,
                            activation="Relu", name="Conv1", trainable=True):
        """
        ...
        self.model.add(keras.layers.Conv2D(filters=num_of_filters, kernel_size=kernel_size,
                                           strides=strides, padding=padding,  activation=activation, name=name, trainable=trainable))
    ...
    def append_maxpooling2d_layer(self, pool_size=2, padding="same", strides=2, name=""):
        """
         ...
         :return: Layer object
        """
        self.model.add(keras.layers.MaxPooling2D(pool_size=pool_size,
    ...
    def get_weights_without_biases(self, layer_number=None, layer_name=""):
        ...
        """
        try:
            if layer_number and layer_number > 0 and layer_name:
                return self.model.get_layer(name=layer_name, index=int(layer_number - 1)).get_weights()[0]
            elif layer_name:
                ...
                if layer_number == 0:
                    ...
                    return self.model.get_layer(index=int(layer_number - 1)).get_weights()[0]
        except:
            pass
    ...
    def get_biases(self, layer_number=None, layer_name=""):
        ...
        try:
            if layer_number and layer_number > 0 and layer_name:
                return self.model.get_layer(name=layer_name, index=int(layer_number - 1)).get_weights()[1]
            elif layer_name:
                ...
                if layer_number == 0:
                    ...
                    return self.model.get_layer(index=int(layer_number - 1)).get_weights()[1]
        except:
            pass
    ...
    def set_weights_without_biases(self, weights, layer_number=None, layer_name=""):
        ...
        if layer_number and layer_number >= 0 and layer_name:
            self.model.get_layer(index=layer_number, name=layer_name).set_weights(
                [weights, biases])
        elif layer_name:
            self.model.get_layer(name=layer_name).set_weights(
                [weights, biases])
        else:
            self.model.get_layer(index=layer_number).set_weights(
                [weights, biases])
    ...
    def load_a_model(self, model_name="", model_file_name=""):
        ...
        if model_name:
            self.model = eval(
                f"keras.models.Sequential(tf.keras.applications.{model_name}().layers)")
        else:
            self.model = keras.models.Sequential(
                keras.models.load_model(model_file_name).layers)
    ...
    def set_optimizer(self, optimizer="SGD", learning_rate=0.01, momentum=0.0):
        ...
        elif optimizer in ["RMSprop", "Adagrad"]:
            self.optimizer = eval(
                f"keras.optimizers.{optimizer}(learning_rate=learning_rate)")
...
if __name__ == "__main__":
    ...
    my_cnn.append_conv2d_layer(num_of_filters=16, kernel_size=(
        3, 3), padding="same", activation='linear', name="conv1")
    my_cnn.append_maxpooling2d_layer(
        pool_size=2, padding="same", strides=2, name="pool1")
    my_cnn.append_conv2d_layer(
        num_of_filters=8, kernel_size=3, activation='relu', name="conv2")
    ...
    print("dense1 weights: ",
          None if weights is None else weights.shape, type(weights))
